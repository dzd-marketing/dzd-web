...
{/* Chat Toggle Button */} 
  <button ref={chatToggleRef} onClick={toggleChat} 
    ...
    {isOpen ? ( <X size={24} className="absolute transition-all duration-300" /> ) : ( <> <MessageCircle size={24} className="absolute transition-all duration-300 group-hover:scale-110" /> <span className="absolute -top-1 -right-1 w-3 h-3 bg-green-500 border-2 border-white dark:border-slate-900 rounded-full animate-pulse"></span> </> )} </button>
